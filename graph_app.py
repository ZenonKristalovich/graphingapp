...
from data_handler import read_csv
...
class GraphApp(QWidget):
    ...
    def __init__(self, filenames=None, width=7, height=2, dpi=100):
        ...
        # Load data
        self.matrix, self.row_names,self.temperatures,self.components = read_csv(self.filenames)
        ...
        self.graph_title = QLabel(f'Graph Image {self.current_file_index + 1}/{self.total_graphs}')
    ...
    def setup_labels(self, grid):
        ...
        lbl.setFixedWidth(int(150*self.width))
        lbl.setFixedHeight(int(40*self.height))
        grid.addWidget(lbl, 14 + self.components, 1)
    ...
    def setup_buttons(self, grid):
        ...
        button = QPushButton("-")
        button.setFixedWidth(int(150*self.width))
        self.grid.addWidget(button, 14 + self.components, 2)
        self.buttons.append(button)

        button = QPushButton("+")
        button.setFixedWidth(int(150*self.width))
        self.grid.addWidget(button, 14 + self.components, 3)
        self.buttons.append(button)
    ...
    def plot_data(self):
        draw_plot(self.canvas, self.matrix, self.row_names, self.current_file_index, 
